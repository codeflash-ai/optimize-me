...
def find_leaf_nodes(nodes: list[dict], edges: list[dict]) -> list[dict]:
    # Build a set of all node ids that have outgoing edges
    outgoing_sources = {edge["source"] for edge in edges}
    # Nodes whose id is not in outgoing_sources are leaves
    leaf_nodes = [node for node in nodes if node["id"] not in outgoing_sources]
    return leaf_nodes
